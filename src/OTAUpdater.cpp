...
namespace {
        ...
        int openWithRedirects(int maxRedirects = 5) {
            ...
            while (true) {
                ...
                if (lastOpenErr != ESP_OK) {
                    captureSocketErrno();
                    ESP_LOGE(TAG, "HTTP open failed: %s (errno=%d)", esp_err_to_name(lastOpenErr), lastSocketErrno);
                    return -1;
                }
                lastHeadersResult = esp_http_client_fetch_headers(handle);
                if (lastHeadersResult < 0) {
                    captureSocketErrno();
                    ESP_LOGE(TAG, "HTTP fetch_headers failed: %d (errno=%d, esp_err=%s)", lastHeadersResult,
                             lastSocketErrno, esp_err_to_name(lastOpenErr));
                    return -1;
                }
    ...
    struct EspHttpReader {
        ...
        int read() {
            ...
            char c;
            if (int n = esp_http_client_read(handle, &c, 1); n <= 0) {
                eof = true;
                return -1;
            }
    ...
    struct InProgressGuard {
        ...
        ~InProgressGuard() {
            if (armed) otaState().updateInProgress.store(false);
        }
    ...
    RawDnsResult rawDnsQuery(uint32_t serverIp4, const char* name, uint32_t timeoutMs) {
        ...
        query[0] = 0x4c;
        query[1] = 0x5a; // transaction id ("LZ")
        query[2] = 0x01;
        query[3] = 0x00; // standard query, recursion desired
        query[4] = 0x00;
        ...
        n += qn;
        query[n++] = 0x00;
        query[n++] = 0x01; // qtype = A
        query[n++] = 0x00;
        query[n++] = 0x01; // qclass = IN
    ...
    bool probeGetAddrInfo(const char* host, int family, const char* familyName) {
        ...
        ESP_LOGD(TAG, "OTA diag: getaddrinfo(%s, %s) FAILED gai=%d after %lu ms", host, familyName, gai,
                 (unsigned long)elapsed);
        return false;
    }
    ...
    bool logRawDnsQuery(uint32_t serverIp4, const char* host, const char* serverLabel) {
        ...
        switch (raw.outcome) {
            case RawDnsResult::Outcome::Answered:
                ESP_LOGD(TAG, "OTA diag: raw DNS A? %s @%s -> rcode=%d answers=%d in %lu ms", host, serverLabel,
                         raw.rcode, raw.answerCount, (unsigned long)raw.elapsedMs);
                break;
            case RawDnsResult::Outcome::NoReply:
                ESP_LOGD(TAG, "OTA diag: raw DNS A? %s @%s got NO REPLY in %lu ms (errno=%d)", host, serverLabel,
                         (unsigned long)raw.elapsedMs, raw.err);
                break;
            case RawDnsResult::Outcome::SocketFailed:
                ESP_LOGD(TAG, "OTA diag: raw DNS A? %s @%s could not open a socket (errno=%d)", host, serverLabel,
                         raw.err);
                break;
            case RawDnsResult::Outcome::SendFailed:
                ESP_LOGD(TAG, "OTA diag: raw DNS A? %s @%s sendto failed (errno=%d) — no route", host, serverLabel,
                         raw.err);
                break;
            case RawDnsResult::Outcome::Malformed:
                ESP_LOGD(TAG, "OTA diag: raw DNS A? %s @%s reply malformed/too short", host, serverLabel);
                break;
        }
    ...
    bool logTcpProbe(uint32_t ip4, uint16_t port, const char* label) {
        ...
        if (int rc = connect(fd, reinterpret_cast<struct sockaddr*>(&dst), sizeof(dst)); rc == 0) {
            ESP_LOGD(TAG, "OTA diag: tcp %s connected immediately (%lu ms)", label, (unsigned long)(millis() - start));
            close(fd);
            return true;
        }
        ...
        if (sel > 0) {
            ...
            if (soErr == 0) {
                ESP_LOGD(TAG, "OTA diag: tcp %s CONNECTED in %lu ms (host reachable)", label, (unsigned long)elapsed);
                reachable = true;
            } else {
                reachable = (soErr == ECONNREFUSED);
                ESP_LOGD(TAG, "OTA diag: tcp %s refused/failed in %lu ms (SO_ERROR=%d)%s", label,
                         (unsigned long)elapsed, soErr, reachable ? " — host is reachable, port closed" : "");
            }
        } else if (sel == 0) {
            ESP_LOGD(TAG, "OTA diag: tcp %s TIMED OUT after %lu ms — no response at all", label,
                     (unsigned long)elapsed);
        } else {
    ...
    __attribute__((noinline)) void explainNetworkFailure(const char* host) {
        ...
        if (probeGetAddrInfo(host, AF_INET, "AF_INET")) {
            ESP_LOGW(TAG,
                     "OTA verdict: %s resolves, so the failure is in the TLS/HTTP "
        ...
        uint32_t server0 = 0;
        {
            ...
            for (uint8_t i = 0; i < 3; ++i) {
                ...
                off += snprintf(servers.data() + off, servers.size() - off, "%s%s", i ? "," : "", s.toString().c_str());
                if (off >= servers.size()) break;
            }
            ESP_LOGD(TAG, "OTA diag: resolvers=[%s] reset_reason=%d sntp_sync=%d", servers.data(),
                     (int)esp_reset_reason(), (int)sntp_get_sync_status());
        }
        ...
        if (!socketsOk) {
            ...
            ESP_LOGW(TAG,
                     "OTA verdict: gateway %s does not respond even over TCP. The WiFi "
            ...
            ESP_LOGW(TAG,
                     "OTA verdict: gateway %s is reachable but nothing upstream is — "
                     ...
                     WiFi.gatewayIP().toString().c_str(), WiFi.localIP().toString().c_str(), WiFi.macAddress().c_str());
    ...
    bool doCheckForUpdate(const char* owner, const char* repo, FirmwareInfo& out) {
        ...
        if (wl_status_t wifi = WiFi.status(); wifi != WL_CONNECTED) {
            ESP_LOGD(TAG, "GitHub API check skipped: WiFi not connected (status=%d, mode=%d, heap=%u)", (int)wifi,
        ...
        // 1700000000 = 2023-11-14. Anything past 2023 is the modern web's floor.
        {
            ...
            if (!synced) {
                ...
                ESP_LOGW(TAG,
                         "OTA check aborted: system clock not synced after 50s total "
            ...
            ESP_LOGD(TAG, "OTA diag: rssi=%d ch=%d ip=%s sys_time=%s (%ld)", (int)WiFi.RSSI(), (int)WiFi.channel(),
                     WiFi.localIP().toString().c_str(), buf.data(), (long)sys_time);
        }

        ESP_LOGD(TAG, "OTA diag: dns=%s gw=%s", WiFi.dnsIP().toString().c_str(), WiFi.gatewayIP().toString().c_str());
#endif
        ...
        if (int status = client.openWithRedirects(); status != 200) {
            ...
            if (client.lastSocketErrno == 0 || client.lastSocketErrno == EINPROGRESS) {
                ...
                ESP_LOGW(TAG, "GitHub API returned %d (free heap: %u, errno=%d)", status, ESP.getFreeHeap(),
                         client.lastSocketErrno);
            }
            client.drain();
            SemaphoreHandle_t m = checkResultMutex();
            if (xSemaphoreTake(m, portMAX_DELAY)) {
        ...
        if (out.changelog.length() > CHANGELOG_MAX_BYTES) {
            ...
            out.changelog =
                out.changelog.substring(0, CHANGELOG_MAX_BYTES) + "+ \xe2\x80\xa6 (" + String(extra) + " more bytes)";
        }
        ...
        if (out.version.isEmpty() || out.downloadUrl.isEmpty() || out.size == 0) {
            ESP_LOGW(TAG, "No usable release found (tag=%s, url=%s, size=%zu)", out.version.c_str(),
                     out.downloadUrl.c_str(), out.size);
            SemaphoreHandle_t m = checkResultMutex();
            if (xSemaphoreTake(m, portMAX_DELAY)) {
                otaState().checkState = CheckState::Failed;
                xSemaphoreGive(m);
        ...
        ESP_LOGI(TAG, "Check OK: %s (%s, %zu bytes, %u changelog bytes)", out.name.c_str(), out.version.c_str(),
                 out.size, out.changelog.length());
        return true;
    }
    ...
    bool doPerformUpdate(const String& downloadUrl, size_t expectedSize,
        ...
        while (totalRead < expectedSize) {
            if (int n =
                    esp_http_client_read(client.handle, reinterpret_cast<char*>(buffer.data()), DOWNLOAD_CHUNK_SIZE);
                n > 0) {
    ...
    void otaWorkerTask(void* arg) {
        ...
        auto onProgress = [](int percent, size_t bytes) { OTAUpdater::publishProgress(percent, bytes); };
...
bool OTAUpdater::startBackgroundCheck(const char* owner, const char* repo) {
    ...
    if (BaseType_t rc = xTaskCreatePinnedToCore(otaCheckTask, "ota_check", OTA_CHECK_TASK_STACK, job, 1, nullptr, 1);
        rc != pdPASS) {
        delete job;
...
bool OTAUpdater::startBackgroundUpdateFromLatestCheck(bool force) {
    ...
    if (!force) {
        ...
        if (!ota::isNewerVersion(latest, current)) {
            otaState().updateInProgress.store(false);
            ESP_LOGW(TAG, "startBackgroundUpdate refused: %s is not newer than %s (force=false)", info.version.c_str(),
                     FIRMWARE_VERSION);
            return false;
        }
    } else {
    ...
    if (BaseType_t rc = xTaskCreatePinnedToCore(otaWorkerTask, "ota_update", OTA_UPDATE_TASK_STACK, job, 1, nullptr, 1);
        rc != pdPASS) {
        delete job;
        otaState().updateInProgress.store(false);
...
void OTAUpdater::otaCheckTaskEntry(void* arg) {
    otaCheckTask(arg);
}
void OTAUpdater::otaWorkerTaskEntry(void* arg) {
    otaWorkerTask(arg);
}
