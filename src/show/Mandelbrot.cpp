...
std::pair<float, float> Show::Mandelbrot::Mandelbrot::func(float zre, float zim, float cre, float cim) {
    // z_n+1 = z_n^2 + c
    return std::pair<float, float>(zre * zre - zim * zim + cre, 2 * zre * zim + cim);
}

Show::Mandelbrot::Mandelbrot::Mandelbrot(float cReMin, float cImMin, float cImMax, unsigned int scale,
...
void Show::Mandelbrot::Mandelbrot::execute(Strip::Strip &strip, Iteration iteration) {
