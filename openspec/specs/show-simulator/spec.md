...
`scripts/build_pages.py --all` SHALL consult a variants manifest at `scripts/show_variants.json` to decide which parameter sets to render for each registered show. The manifest SHALL be a JSON object keyed by show name, where each value carries a `description` and a `variants` array. Each variant entry SHALL have a unique `name`, a human-readable `label`, and a `params` object that is passed verbatim to the simulator via `--params`. A show that is omitted from the manifest SHALL still render a single default variant with empty parameters so the gallery stays complete.
