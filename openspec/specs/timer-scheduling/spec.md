...
## Purpose
Defines how the device keeps local time and evaluates scheduled actions: the POSIX timezone representation that drives all local-time values, the behaviour of countdown timers and daily alarms across daylight-saving transitions, the HTTP surface for reading and writing the timezone, the timers page controls for picking a timezone by place, and the one-time migration of legacy offset and alarm state.
