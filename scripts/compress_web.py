...
JS_PAYLOAD_PATH = GENERATED_DIR / "show_variants.js"
JS_INLINE_MARKER = "<!--SHOW_VARIANTS_INLINE-->"
...
def inline_js_payload(html: str) -> str:
    """Replace the JS_INLINE_MARKER with a <script> carrying show_variants.js.

    The payload is regenerated by scripts/gen_show_variants.py on every
    PlatformIO build before this script runs. If the payload is missing the
    marker cannot be substituted, which would surface as a leftover comment
    in the rendered HTML; raise a clear error so the cause is obvious.
    """
    if not JS_PAYLOAD_PATH.exists():
        raise FileNotFoundError(
            f"{JS_PAYLOAD_PATH} not found; run scripts/gen_show_variants.py first"
        )
    js_contents = JS_PAYLOAD_PATH.read_text(encoding="utf-8").rstrip()
    inline = f"<script>\n{js_contents}\n</script>"
    if JS_INLINE_MARKER not in html:
        raise ValueError(
            f"{JS_INLINE_MARKER} marker not found in HTML; expected exactly one occurrence"
        )
    return html.replace(JS_INLINE_MARKER, inline, 1)
...
def process_file(file_path: Path) -> tuple[str, int, int]:
    ...
    # Inline the generated JS payload (consumed before HTML comment removal so
    # the marker is replaced before minify_html() runs).
    if file_path.suffix == '.html' and JS_INLINE_MARKER in content:
        content = inline_js_payload(content)
